...
class PessoaFisicaDAO{
	...
	function cadastrar(PessoaFisica $objPessoaFisica){
		$this->sql = sprintf("INSERT INTO pessoafisica (nome, cpf, datanascimento, estadocivil, sexo, nomepai, nomemae, cor, naturalidade, nacionalidade, importado) VALUES('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', %d)",
				...
				mysqli_real_escape_string( $this->con, $objPessoaFisica->getImportado() ));
	...
	/*-- Metodo Atualizar --*/
	function atualizarImportado(PessoaFisica $objPessoaFisica){
		$this->sql = sprintf("UPDATE pessoafisica SET importado = %d WHERE id = %d",
				mysqli_real_escape_string( $this->con, $objPessoaFisica->getImportado() ),
				mysqli_real_escape_string( $this->con, $objPessoaFisica->getId() ) );
	...
	function buscarPorId(PessoaFisica $objPessoaFisica){
		...
		while($row = mysqli_fetch_object($resultSet)){
			$this->objPessoaFisica = new PessoaFisica($row->id, $row->nome, $row->cpf, $row->datanascimento, $row->estadocivil, $row->sexo, $row->nomepai, $row->nomemae, $row->cor, $row->naturalidade, $row->nacionalidade, $row->datacadastro, $row->dataedicao, $row->importado);
		}
	...
	/*-- listaPessoaFisica Nao importados paginado  --*/
	function listarNaoImportadosPaginado($start, $limit) {
		$this->sql = "SELECT * FROM pessoafisica WHERE importado = 1 limit " . $start . ", " . $limit;
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
			die ( '[ERRO]: ' . mysqli_error ( $this->con ) );
	...
	function qtdTotalNaoImportados() {
		$this->sql = "SELECT count(*) as quantidade FROM pessoafisica WHERE importado = 1";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
			die ( '[ERRO]: ' . mysqli_error ( $this->con ) );
		}
