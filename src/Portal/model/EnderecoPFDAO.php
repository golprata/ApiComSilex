...
class EnderecoPFDAO{
	...
	function cadastrar(EnderecoPF $objEnderecoPF){
		$this->sql = sprintf("INSERT INTO enderecopf (idtipoendereco, logradouro, numero, complemento, bairro, cep, cidade, idlocalidade, idpessoafisica)
				...
				mysqli_real_escape_string( $this->con, $objEnderecoPF->getCidade() ),
	...
	function atualizar(EnderecoPF $objEnderecoPF){
		$this->sql = sprintf("UPDATE enderecopf SET idtipoendereco = %d, logradouro = '%s', numero = '%s', complemento = '%s', bairro = '%s', cep = '%s', cidade = '%s', idlocalidade = %d, idpessoafisica = %d WHERE id = %d",
				mysqli_real_escape_string( $this->con, $objEnderecoPF->getObjtipoendereco()->getId() ),
				mysqli_real_escape_string( $this->con, $objEnderecoPF->getLogradouro() ),
				mysqli_real_escape_string( $this->con, $objEnderecoPF->getNumero() ),
				mysqli_real_escape_string( $this->con, $objEnderecoPF->getComplemento() ),
	...
	function buscarPorId(EnderecoPF $objEnderecoPF){
		...
		while($row = mysqli_fetch_object($resultSet)){
			...
			$this->objEnderecoPF = new EnderecoPF($row->id, $objTipoEndereco, $row->logradouro, $row->numero, $row->complemento, $row->bairro, $row->cep, $row->cidade, $objLocalidade, $objPessoaFisica, $row->datacadastro, $row->dataedicao);
		}
